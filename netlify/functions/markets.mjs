...
export default async (request) => {
  ...
  try {
    ...
    // Debug: capture raw count and sample before filtering
    const rawCount = markets.length;
    const rawSample = markets.slice(0, 3).map(m => ({
      ticker: m.ticker,
      status: m.status,
      yes_bid: m.yes_bid,
      yes_ask: m.yes_ask,
    ...
    // Filter for open markets with some activity
    markets = markets.filter(m => {
      const isOpen = m.status === "open";
      const hasVolume = (m.volume || 0) >= minVolume;
      const hasPrice = (m.yes_bid > 0 || m.yes_ask > 0 || m.last_price > 0);
      return isOpen && hasVolume && hasPrice;
    });
