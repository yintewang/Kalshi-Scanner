...
export default async (request) => {
  ...
  try {
    ...
    // Use mve_filter=exclude to skip multivariate combo/parlay markets.
    // Fetch in batches — first a broad sweep, then targeted series.
    let allMarkets = [];

    // Broad fetch excluding combos
    let cursor = "";
    for (let page = 0; page < 5; page++) {
      let marketsUrl = KALSHI_BASE + "/markets?limit=1000&status=open&mve_filter=exclude";
    ...
    // Filter for markets with some trading activity
    // Note: Kalshi returns status="active" for open markets despite docs saying "open"
    markets = markets.filter(m => {
      const hasPrice = dollarsToCents(m.yes_bid_dollars) > 0 ||
                       dollarsToCents(m.yes_ask_dollars) > 0 ||
                       dollarsToCents(m.last_price_dollars) > 0;
      return hasPrice;
    });
    ...
    // Apply minimum volume filter after sorting (volume_fp is in contracts)
    if (minVolume > 0) {
      markets = markets.filter(m => dollarsToFloat(m.volume_fp) >= minVolume);
    }

    // Also filter out markets that are clearly combos by ticker prefix
    markets = markets.filter(m => {
      const t = (m.ticker || "").toUpperCase();
      return !t.startsWith("KXMVE");
    });
    ...
    const categorized = markets.map(m => {
      ...
      else if (matchesCategory(title, ticker, "sports")) cat = "sports";
      ...
      return {
        ...
        contractName: m.yes_sub_title || m.subtitle || m.ticker,
        category: cat,
        ...
        kalshiUrl: "https://kalshi.com/events/" + (m.event_ticker || m.ticker || ""),
      };
    });
    ...
    // Return a balanced mix across categories instead of pure volume sort
    // This prevents sports from crowding out crypto/economics/politics
    if (category === "all") {
      const buckets = {};
      categorized.forEach(m => {
        if (!buckets[m.category]) buckets[m.category] = [];
        buckets[m.category].push(m);
      });
      // Sort each bucket by volume
      Object.keys(buckets).forEach(k => {
        buckets[k].sort((a, b) => b.volume - a.volume);
      });
      // Take top markets from priority categories first
      const priority = ["crypto", "economics", "politics", "sports", "other"];
      filtered = [];
      // First pass: top 15 from each priority category
      priority.forEach(cat => {
        if (buckets[cat]) {
          filtered = filtered.concat(buckets[cat].slice(0, 15));
        }
      });
    ...
    // Compute basic fair value estimates (first pass)
    ...
    // Second pass: fetch live order book for markets with potential edges
    // This gets the real-time best bid/ask instead of stale last-trade data
    const marketsWithEdge = enriched.filter(m => m.edge !== null && Math.abs(m.edge) >= 3);
    const orderbookFetches = marketsWithEdge.slice(0, 10).map(async (m) => {
      try {
        const obUrl = KALSHI_BASE + "/markets/" + m.ticker + "/orderbook";
        const obRes = await fetch(obUrl, { headers: { "Accept": "application/json" } });
        if (!obRes.ok) return;
        const obData = await obRes.json();
        const ob = obData.orderbook || {};

        // Get best yes bid and best yes ask from the order book
        const yesBids = (ob.yes || []).sort((a, b) => b[0] - a[0]); // highest first
        const noBids = (ob.no || []).sort((a, b) => b[0] - a[0]);
        ...
        if (bestYesBid > 0 || bestYesAsk > 0) {
          m.yesBid = bestYesBid;
          m.yesAsk = bestYesAsk > 0 ? bestYesAsk : 0;

          // Recalculate market price from live order book
          let livePrice;
          if (bestYesBid > 0 && bestYesAsk > 0) {
            livePrice = Math.round((bestYesBid + bestYesAsk) / 2);
          } else if (bestYesAsk > 0) {
            livePrice = bestYesAsk;
          } else {
            livePrice = bestYesBid;
          }
          m.marketPrice = livePrice;

          // Recalculate edge with live price
          if (m.fairValue !== null) {
            m.edge = m.fairValue - m.marketPrice;
          }
          m.fairSource = m.fairSource + "+orderbook";
        }
      } catch {}
    });
    ...
    // Deduplicate by event ticker — keep the contract with the largest absolute edge
    // This prevents the same event showing up multiple times with different outcomes
    const dedupMap = new Map();
    enriched.forEach(m => {
      const key = m.eventTicker || m.ticker;
      const existing = dedupMap.get(key);
      const absEdge = m.edge !== null ? Math.abs(m.edge) : -1;
      const existingEdge = existing && existing.edge !== null ? Math.abs(existing.edge) : -1;
      if (!existing || absEdge > existingEdge) {
        dedupMap.set(key, m);
      }
    });
    const deduped = Array.from(dedupMap.values());

    // Category breakdown for debug
    const catCounts = {};
    categorized.forEach(m => { catCounts[m.category] = (catCounts[m.category] || 0) + 1; });
...
function matchesCategory(title, ticker, category) {
  const keywords = {
    ...
    sports: ["wins by over", "points scored", "total points", "spread", "nba", "nfl", "nhl", "mlb", "ncaa", "duke", "kansas", "march madness", "playoff", "super bowl", "goals scored", "assists", "rebounds", "touchdowns", "rushing", "passing", "home run"],
  };
