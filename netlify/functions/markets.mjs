  ...
  try {
    ...
    const categorized = markets.map(m => {
      ...
      return {
        ...
        // Kalshi URL: /markets/{series_ticker} — extract series from event_ticker (before first dash)
        kalshiUrl: "https://kalshi.com/markets/" + ((m.event_ticker || m.ticker || "").split("-")[0]).toLowerCase(),
      };
    });
